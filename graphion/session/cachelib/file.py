...
from graphion.session.cachelib.base import BaseCache
from graphion.session.cachelib._compat import text_type
    ...
    def get(self, key):
        ...
        try:
            with open(filename, 'rb') as f:
                dill_time = dill.load(f)
                if dill_time == 0 or dill_time >= time():
                    return dill.load(f)
                else:
                    os.remove(filename)
                    return None
        except (IOError, OSError, dill.dillError):
            return None
    ...
    def set(self, key, value, timeout=None, mgmt_element=False):
        ...
        try:
            ...
            with os.fdopen(fd, 'wb') as f:
                dill.dump(timeout, f, 1)
                dill.dump(value, f, dill.HIGHEST_PROTOCOL)
